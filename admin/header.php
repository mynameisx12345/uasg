...
$pageTitles = [
  ...
  'task-management.php' => 'Task Management',
  'ml-management.php' => 'ML Classification',
  'reports-page.php' => 'Reports & Analytics',
  'account-security.php' => 'Account Security',
];
$currentPage = basename($_SERVER['PHP_SELF']);
$pageTitle = $pageTitles[$currentPage] ?? 'Admin Panel';
$adminName = $_SESSION['full_name'] ?? $_SESSION['username'] ?? 'System Admin';
if (in_array(strtolower(trim($adminName)), ['admin', 'administrator', 'system administrator'])) $adminName = 'System Admin';
  ...
  <div class="topbar-right">
    ...
    <div class="topbar-user">
      ...
      <div class="topbar-user-info">
        ...
        <span class="topbar-role"><?= $adminRole ?></span>
      </div>
    </div>
  </div>
</header>
...
function toggleMobileSidebar(){
  var s=document.querySelector('.sidebar'); var o=document.getElementById('sidebarOverlay');
  if(!s)return;
...
<script>
(function(){
  var CALL_GET=29, CALL_MARK=30, CALL_COUNT=31, USER_ID=<?= (int)($_SESSION['user_id'] ?? 0) ?>;
  var open=false;

  function fetchCount(){
    $.post('ajax.php',{CALL:CALL_COUNT,USER_ID:USER_ID},function(r){
  ...
  function fetchList(){
    $.post('ajax.php',{CALL:CALL_GET,USER_ID:USER_ID},function(r){
      var items=r.data||[];
  ...
  window.markAllNotifsRead=function(){
    $.post('ajax.php',{CALL:32,USER_ID:USER_ID},function(){
      $('#notifBadge').hide();
      fetchList();
    },'json');
  };
